...
def run_antechamber(
    ...
    for index, line in enumerate(iterable=charge_lines[8:num_atoms+8]):
        chg = line.split()[8]
        charges[index] = chg
    ...
    for index, line in enumerate(iterable=coord_lines[8:num_atoms+8]):
        atom_line = line[:70]
    ...
    subprocess.run(args=["sed", "-i", "'s/ Cl/ CL/g'", f"{lig_code}_ac.mol2"], cwd=path, check=True)
