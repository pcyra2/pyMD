...
class MDClass:
    ...
    def __init__(
        ...
        if self._backend == "amber":
            self.kernel = Amber(config = config)
        else:
    ...
    def set_parmfile(
        ...
        """
        self.kernel.set_global(parmfile = parmfile)
    ...
    def make_job(self,
        ...
        """
        self.kernel.config.set_calculation_variables(paramfile = self.kernel.parm_file,
        ...
        self.current_job = MDJobClass(inputfile_name = input_file_name,
                                        ...
                                        run_path = run_path)
        file = self.kernel.config.gen_input_file(filename = input_file_name)
        self.current_job.add_inputfile(inputfile = file)
        self.current_job.add_kernel(config = self.kernel)
    ...
    def minimize(self,
        ...
        Args:
            ...
            hpc_sub (bool): Whether to attach a slurm job to the job. Defaults to False.
        """
        self.kernel.set_ensemble(ensemble = "min", steps = steps, steps_steepest = steps_steepest)
        self.kernel.set_restraints(restraint_mask = restraints,
                                   restraint_wt = self.kernel.config.restraint_wt)

        self.kernel.set_outputs(energy = energy_out, restart = restart_out, trajectory = traj_out)
        ...
        if hpc_sub:
            ...
            slurm_sub = copy.deepcopy(x = self.base_slurm)
            slurm_sub.set_gpus(gpus = 0)
            slurm_sub.define_dirs(local_file_path = run_path, 
                                  hpc_file_path = os.path.join(slurm_sub.hpc_run_dir, run_path))
            slurm_sub.set_name(name = job_name)
            slurm_sub.set_ntasks(tasks = self.num_cpu)
            for module in slurm_sub.modules:
                if "cuda" in module:
                    slurm_sub.modules.remove(module)
            self.current_job.attach_slurm(hpc = slurm_sub)
    ...
    def heat(self,
            ...
            restraints: str|None = None,
            timestep: float = convert.time(in_time = 2, in_unit = "fs", out_unit = "ps"),
        ...
        if hpc_sub:
            if self.base_slurm is None:
                self.base_slurm = Slurm()
            slurm_sub = copy.deepcopy(x = self.base_slurm)
            slurm_sub.set_gpus(gpus = 1)
            slurm_sub.define_dirs(local_file_path = path, 
                                hpc_file_path = os.path.join(slurm_sub.hpc_run_dir, path))
            slurm_sub.set_name(name = job_name)
            slurm_sub.set_ntasks(tasks = self.num_cpu)
            self.current_job.attach_slurm(hpc = slurm_sub)
    ...
    def constant(self,
        ...
        Args:
            ...
            job_name (str): Name of the calulation
            steps (int): Number of MD steps to run.
            temperature (float, optional): Temperature to run the calculation at. Defaults to 300.0.
            thermostat (str | int | None, optional): Defenition of thermostat. 
                Defaults to None which uses the default thermostat.
            pressure (float | None, optional): Pressure to run the calculation at. 
                If none then NVT ensemble is used, else NPT. Defaults to None.
            barostat (str | int | None, optional): The barostat to use, Accepts either the name, 
                or AMBER integer value. Defaults to None which uses the default barostat.
            restraints (str | None, optional): The restraints to use, if any. Defaults to None.
            timestep (float, optional): The MD timestep to use in picoseconds. 
                Defaults to  2 fs. convert.time(2, in_unit="fs", out_unit="ps").
            path (str, optional): The working directory for this calculation. Defaults to "./".
            traj_out (int, optional): The frequency to add to the trajectory file. Defaults to 100.
            energy_out (int, optional): The frequency to add to the energy file. Defaults to 10.
            restart_out (int, optional): The frequency to update the restart file. Defaults to 10.
            hpc_sub (bool, optional): Whether to attach a slurm job to the job. Defaults to False.
        """
